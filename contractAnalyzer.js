  ...
  try {
    ...
    try {
      ...
      if (sourceCode.startsWith("{") && sourceCode.endsWith("}")) {
        ...
        // Attempt to handle additional wrapping issues
        const firstOpenBrace = sourceCode.indexOf('{');
        const lastCloseBrace = sourceCode.lastIndexOf('}');
        sourceCode = sourceCode.substring(firstOpenBrace, lastCloseBrace + 1);
        parsedSourceCode = JSON.parse(sourceCode);
      } else {
      ...
      console.log("Parsed source code successfully.");
    } catch (error) {
      console.error("Failed to parse source code JSON. Error:", error.message);
      return null;
    }
    ...
    for (const file of contractFiles) {
      const absolutePath = updatePath(baseDir, file, Array.from(libraries), nodeModulesPresent);
      try {
...
//runAll('0x6985884C4392D348587B19cb9eAAf157F13271cd');
